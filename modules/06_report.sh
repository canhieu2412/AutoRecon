...
# ── Phase G: exam proof capture ──────────────────────────────────────────────
# Record a local.txt / proof.txt flag with the context OffSec wants (target IP,
# whoami, ipconfig/ifconfig) into results/<t>/proof/. Non-destructive.
report_capture_proof() {
    local ip="$1" result_dir="$2"
    local pdir="${result_dir}/proof"
    mkdir -p "$pdir"
    local which_flag flag localip
    if declare -F _sh_ask >/dev/null; then
        which_flag=$(_sh_ask "Flag type (local/proof)" "proof")
        flag=$(_sh_ask "Flag value (paste hash from local.txt/proof.txt)" "")
        localip=$(_sh_ask "Target's own IP (ip a / ipconfig)" "$ip")
    else
        echo -ne "  Flag type (local/proof) [proof]: "; read -r which_flag; which_flag="${which_flag:-proof}"
        echo -ne "  Flag value: "; read -r flag
        echo -ne "  Target IP [${ip}]: "; read -r localip; localip="${localip:-$ip}"
    fi
    [[ -z "$flag" ]] && { log_warn "No flag entered — nothing saved."; return 1; }
    local ts; ts=$(date '+%Y-%m-%d %H:%M:%S %Z')
    local out="${pdir}/${which_flag}_${ip}.txt"
    {
        echo "=== ${which_flag}.txt proof — ${ip} ==="
        echo "captured: ${ts}"
        echo "target-reported IP: ${localip}"
        echo "flag: ${flag}"
        echo ""
        echo "# OSCP note: screenshot the shell showing BOTH  \`type|cat <flag path>\`  AND"
        echo "#  \`ipconfig\`/\`ip a\` in the SAME window. This file is just a text backup."
    } > "$out"
    log_success "Proof saved → ${out}"
    return 0
}

# ── Phase G: OffSec-style submission template ────────────────────────────────
report_generate_oscp_template() {
    local ip="$1" result_dir="$2"
    local out="${result_dir}/oscp_submission.md"
    local ports=""; [[ -s "${result_dir}/scans/open_ports.txt" ]] && ports=$(cat "${result_dir}/scans/open_ports.txt")
    cat > "$out" <<EOF
# OSCP Exam / Lab Submission — ${ip}

> Template auto-generated by Auto Recon. Fill the blanks, keep every command +
> screenshot. Each machine section must prove the flag WITH \`ip a\`/\`ipconfig\`
> visible in the same screenshot.

## 1. Summary
- **Target**: ${ip}
- **Open ports**: ${ports:-<from scans/open_ports.txt>}
- **Initial foothold**: <service + vuln>
- **Privilege escalation**: <technique>

## 2. Enumeration
\`\`\`
# nmap / service scan output — see scans/ and next_steps.txt
\`\`\`
_Screenshot:_ nmap results.

## 3. Initial Access (proof of exploitation)
- **Vulnerability**: <name / CVE>
- **Steps**:
  1. <command>
  2. <command>
\`\`\`
# exploit command + response
\`\`\`
_Screenshot:_ shell as low-priv user + \`whoami\`.

### local.txt / user flag
\`\`\`
cat local.txt      # + ip a  (same screenshot)
\`\`\`
Flag: \`<paste — also saved under proof/>\`

## 4. Privilege Escalation
- **Technique**: <sudo / SUID / kernel / service> (see privesc/ + gtfobins_lolbas.txt)
- **Steps**:
\`\`\`
# escalation command(s)
\`\`\`
_Screenshot:_ root/SYSTEM shell + \`whoami\`.

### proof.txt / root flag
\`\`\`
cat /root/proof.txt      # + ip a  (same screenshot)
\`\`\`
Flag: \`<paste — also saved under proof/>\`

## 5. Post-Exploitation Notes
- Credentials found: <...>
- Pivot routes (see pivot/): <...>

---
_Artifacts_: report.md · commands_poc.txt · next_steps.txt · privesc/ · proof/
EOF
    log_success "OSCP submission template → ${out}"
    return 0
}
...
run_report() {
    ...
    # Phase G: always drop a fill-in OSCP submission template alongside the report.
    report_generate_oscp_template "$ip" "$result_dir"

    echo ""
    echo -e "  ${BOLD}${GREEN}View report: cat ${report}${NC}"
    echo -e "  ${BOLD}${GREEN}HTML report: xdg-open ${html_report}${NC}"
    echo -e "  ${BOLD}${GREEN}OSCP template: ${result_dir}/oscp_submission.md${NC}"
}
